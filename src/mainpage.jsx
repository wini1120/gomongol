      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-gray-100 sticky top-0 h-screen p-8 justify-between z-50">
        <div className="space-y-10">
          ...
          <nav className="space-y-2">
            <div className="flex items-center gap-3 p-4 bg-gmg-bg text-gmg-camel rounded-2xl font-black cursor-pointer"><Home size={20} /> 홈</div>
            <div className="flex items-center gap-3 p-4 text-gray-400 hover:text-gmg-camel transition-all font-bold cursor-pointer"><Users size={20} /> 동행찾기 게시판</div>
            <div className="flex items-center gap-3 p-4 text-gray-400 hover:text-gmg-camel transition-all font-bold cursor-pointer"><MessageSquareText size={20} /> 여행 후기 게시판</div>
          </nav>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto bg-white lg:bg-transparent">
        ...
        <div className="max-w-4xl mx-auto w-full px-6 py-10 lg:py-20">
          ...
          <div className="space-y-4 mb-16 lg:mb-24">
            <button onClick={onStartBuilder} className="w-full bg-gmg-camel text-white p-7 lg:p-10 rounded-[2.5rem] flex items-center justify-between shadow-xl shadow-orange-100 transition-all active:scale-[0.98]">
              <div className="flex items-center gap-5 text-left">
                ...
                <div>
                  ...
                  <span className="text-sm opacity-90 font-medium tracking-tight">나만의 취향저격 루트 생성</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3 lg:gap-4">
              <button onClick={onStartExplorer} className="bg-white border-2 border-orange-50 py-4 lg:py-6 rounded-[1.8rem] lg:rounded-[2.5rem] flex flex-col items-center justify-center gap-1.5 shadow-sm hover:border-gmg-camel transition-all active:scale-95">
                <div className="bg-orange-50 p-2 lg:p-3 rounded-xl lg:rounded-2xl"><Search size={18} className="text-gmg-camel" /></div>
                <span className="text-[10px] lg:text-sm font-black text-gray-600 tracking-tighter">몽골 둘러보기</span>
              </button>
              <button className="bg-white border-2 border-gmg-green/10 py-4 lg:py-6 rounded-[1.8rem] lg:rounded-[2.5rem] flex flex-col items-center justify-center gap-1.5 shadow-sm hover:border-gmg-green transition-all active:scale-95">
                <div className="bg-gmg-green/5 p-2 lg:p-3 rounded-xl lg:rounded-2xl"><Users size={18} className="text-gmg-green" /></div>
              ...
              <button className="bg-white border-2 border-blue-50 py-4 lg:py-6 rounded-[1.8rem] lg:rounded-[2.5rem] flex flex-col items-center justify-center gap-1.5 shadow-sm hover:border-blue-400 transition-all active:scale-95">
                <div className="bg-blue-50 p-2 lg:p-3 rounded-xl lg:rounded-2xl"><MessageSquareText size={18} className="text-blue-400" /></div>
          ...
          <section className="mb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
              {agencies.map(agency => (
                <div key={agency.id} className="bg-white p-7 lg:p-8 rounded-[2.8rem] border border-gray-50 shadow-sm hover:shadow-md transition-all">
                  <div className="flex items-center gap-5">
                    <div className={`w-14 h-14 ${agency.color} rounded-2xl flex items-center justify-center shrink-0`}><Building2 size={24} className="text-gray-400" /></div>
                    <div>
                      ...
                      <div className="flex items-center gap-2"><Star size={14} className="text-orange-400 fill-orange-400" /><span className="text-sm font-bold text-gray-600">{agency.rating}</span><span className="text-[10px] text-gray-300 font-bold uppercase ml-1">Reviews {agency.reviews}</span></div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <footer className="py-20 border-t border-gray-100 text-center lg:text-left flex flex-col lg:flex-row justify-between items-center gap-8">
            <div className="space-y-2">
              <p className="text-[12px] font-black text-gray-300 uppercase tracking-[0.2em]">Made by Go몽골 | <span onClick={() => setIsContactOpen(true)} className="cursor-pointer hover:text-gmg-camel underline transition-colors">Contact Us</span></p>
              <p className="text-[11px] text-gray-200 font-medium">© 2026 GoMongol. All rights reserved.</p>
            </div>
          </footer>
        </div>
      </main>
